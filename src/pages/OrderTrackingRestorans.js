...
const CountdownTimer = ({ orderTime, prepTime }) => {
    ...
    useEffect(() => {
        const calculateTimeLeft = () => {
            ...
            const target = start + prepTime * 60 * 1000;
            const now = Date.now();
                    ...
                    <table>
                        ...
                        {deliveredOrders.map(o => (
                            <tr key={o.orderId}>
                                ...
                                <td>{o.items.map((i, idx) => <div key={idx}>{i.itemName} x {i.quantity}</div>)}</td>
                    ...
                    <table>
                        ...
                        {orders.map(o => (
                            <tr key={o.orderId}>
                                ...
                                <td>{o.status}</td>
                                <td>
                                    {o.status === "PREPARING" && o.estimatedPreparationTime
                                        ? <CountdownTimer orderTime={o.orderTime} prepTime={o.estimatedPreparationTime} />
                                        : o.estimatedPreparationTime
                                            ? `${o.estimatedPreparationTime} minutes`
                                            : "N/A"}
                                </td>
                                <td>
                                    <button
