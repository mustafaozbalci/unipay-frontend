...
const Dashboard = ({user, onBalanceUpdate, setUser, addNotification}) => {
    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            try {
                ...
                setUser({
                    id: data.id, name: data.username, email: data.email, balance: data.balance, plate: data.plate || ""
                });
            } catch (err) {
    ...
    const isRegular = !isRestaurant && !isOtopark;
    ...
    return (<div className="dashboard-container">
        <div className="transparent-box">
            ...
            {isOtopark && (<div className="user-info">
                <h2>Welcome, {user.name}</h2>
                <p>Balance: {user.balance.toFixed(2)}₺</p>
                <div className="dashboard-buttons">
                    <button
            ...
            {isRegular && (<div className="user-info">
                <h2>Welcome, {user.name}</h2>
                <p>
                ...
                {hasPlate ? (<p>Plate: {user.plate}</p>) : (<button
        ...
        {showSettings && (<div className="settings-modal">
            <div className="modal-content">
                ...
                <form onSubmit={handleSettingsSubmit}>
                    ...
                    <button type="submit">
                        {settingsTab === "password" ? "Update Password" : "Update Plate"}
                    </button>
                </form>
            </div>
        </div>)}
